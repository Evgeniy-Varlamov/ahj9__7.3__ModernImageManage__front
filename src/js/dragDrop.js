...
function get() {
    (async() => {
        {
            const response = await fetch('https://ahj-9-7-3.herokuapp.com/?catalog', {
                method: 'GET',
            });
            ...
            data.forEach((prop) => {
                if (prop !== '.gitkeep') {
                    ...
                    img.src = `https://ahj-9-7-3.herokuapp.com/${prop}`;
                    ...
                    cross.addEventListener('click', async(e) => {
                        ...
                        (async() => {
                            {
                                const response = await fetch(`https://ahj-9-7-3.herokuapp.com/?${prop}`, {
                                    method: 'DELETE',
                                });
                                if (response.ok) {
                                    cross.closest('.element').parentNode.removeChild(cross.closest('.element'))
                                }
                            }
                        })()
                    })
...
area.addEventListener('drop', async(e) => {
    ...
    (async() => {
        ...
        {
            const response = await fetch('https://ahj-9-7-3.herokuapp.com/', {
                method: 'POST',
                body: formData,
            });
        }
